...
void perf_print_all(void) {
  ...
  // Event N lives in mhpmcounter(3+N); ids and names come from pcer_cv32e40p.h.
  // ELW needs COREV_CLUSTER and APU_* need the FPU, otherwise they read 0.
  for (int i = 0; i < CV32E40P_PERF_NB_EVENTS; i++)
    printf("Perf %s: %d\n", CV32E40P_PERF_NAME(i), cpu_perf_get(i));
