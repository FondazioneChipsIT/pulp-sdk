...
static int32_t hyperflash_ioctl(struct pi_device *device, uint32_t cmd, void *arg)
{
  switch (cmd)
  {
    ...
    {
      hyperflash_t *hyperflash = (hyperflash_t *)device->data;
      pi_hyper_ioctl(&hyperflash->hyper_device, PI_HYPER_IOCTL_SET_TRAN_ID, arg);
      break;
    }
  }
  return 0;
...
static inline void hyperflash_bwrite(struct pi_device *device, uint32_t pi_flash_addr, void *buffer, int nb_word)
{
  ...
  int arg = hyperflash_id_alloc(device);
  hyperflash_ioctl(device, PI_FLASH_IOCTL_ID, &arg);

  hyperflash_set_reg_exec(hyperflash, 0x555<<1, 0xAA);
  hyperflash_set_reg_exec(hyperflash, 0x2AA<<1, 0x55);
  hyperflash_set_reg_exec(hyperflash, pi_flash_addr, 0x25);
  hyperflash_set_reg_exec(hyperflash, pi_flash_addr, nb_word-1);

  for(int i=0; i<nb_word; i++)
  {
    hyperflash_set_reg_exec(hyperflash, pi_flash_addr+(i<<1), *((unsigned short*) buffer+i));
  }
...
{
  hyperflash_t *hyperflash = (hyperflash_t *)device->data;
  return pi_hyper_ctl_status_regs(&hyperflash->hyper_device, cmd, arg);
}
...
static pi_flash_api_t hyperflash_api = {
  ...
  .bwrite               = &hyperflash_bwrite,
  .id_alloc             = &hyperflash_id_alloc,
  .status_regs          = &hyperflash_ctl_status_regs,
};
