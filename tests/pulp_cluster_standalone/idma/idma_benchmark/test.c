...
int test_idma_idle (int size) {
    PRINTF ("IDLE TESTCASE --> The transfer is executed by the core while iDMA is gated \n");
    ...
    PRINTF ("The core will execute a transfer instead of the iDMA \n");

    // Let the core do a transfer instead of the DMA
    for (int i=0; i<size; i++) {
        dst_ptr[i] = src_ptr[i];
        // PRINTF ("Putting 0x%8x @0x%8x into  @0x%8x = 0x%8x \n", src_ptr[i], &src_ptr[i], &dst_ptr[i], dst_ptr[i]);
    }
    return 0;
}
...
void allocate_mem_to_cores () {
    ...
    // The following pi_cl_team_barrier is needed so that
...
int cluster_task () {
    ...
    if (core_id == 0){
        PRINTF ("Global errors initialized to %d \n", errors);
        #ifdef TX
